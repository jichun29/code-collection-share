...
STORE_ID = os.getenv("HSAY_STORE_ID", "201424")  # 沪上阿姨门店 id（抓包确认，缺失会导致「品牌Id为空」）
...
def get_code(*_gc_args, **_gc_kwargs) -> str | None:
    """通用取码函数：兼容各种原始签名。

    已见变体（全部由本函数统一吸收）：
        get_code(server)
        get_code(server, appid)
        get_code_for(server, appid)
        get_code(server, phone_code=False)
        get_code(server, appid, phone_code=True)
        get_code(server=..., appid=...)
    appId 优先取 appid/app_id 关键字，其次取位置参数里的 wx 开头项，
    最后回退脚本级 APPID（如有定义）。
    """
    _vals = list(_gc_args)
    for _k in ("server", "host", "code_server"):
        if _k in _gc_kwargs:
            _vals.append(_gc_kwargs.pop(_k))
    _appid = (_gc_kwargs.get("appid") or _gc_kwargs.get("app_id")
              or _gc_kwargs.pop("appid", None) or _gc_kwargs.pop("app_id", None) or "")
    if not _appid:
        for _v in _vals[1:] + list(_gc_kwargs.values()):
            if isinstance(_v, str) and _v.startswith("wx"):
                _appid = _v
                break
    if not _appid:
        _appid = globals().get("APPID") or globals().get("APP_ID") or ""

    _server = _vals[0] if _vals else (globals().get("CODE_SERVER") or "192.168.31.33:5368")
    url = f"http://{_server}/wx/code"
    print(f"🔐 [授权] 请求本地 code 服务: {url} appId={_appid}")

    for _attempt in range(1, 4):
        if _attempt > 1:
            _wait = 5
            print(f"🔄 [重试] {_wait}s 后第 {_attempt}/3 次重试取 code...")
            sleep(_wait)
        try:
            response = direct_session().post(
                url,
                json={"app_id": _appid, "ref": "1"},
                timeout=20,
            )
            data = response.json()
...
def plain_request(
    ...
    try:
        response = request_with_proxy(
            method, url, headers=headers, json=payload_obj, proxies=proxies, server=server,
        )
        try:
        ...
        return {"status": False, "code": -1, "message": f"请求异常: {exc}"}
...
def login_by_code(server: str, code: str, proxies: Dict[str, str] | None) -> Tuple[str | None, Dict[str, Any] | None]:
    try:
        ...
        # 真机为明文 JSON（非 AES-GCM），直接走普通请求
        data = plain_request(
            "POST",
            LOGIN_URL,
            {"code": code, "eVersion": "1.0", "appid": APPID},
            proxies=proxies,
            server=server,
        )
